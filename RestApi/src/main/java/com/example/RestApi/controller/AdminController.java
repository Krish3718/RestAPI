...
public class AdminController {
    ...
    public ResponseEntity<List<Employee>> getAllEmployees(){
        ...
        try {
            ...
            emps = this.adminService.getAllEmployeesService();
    ...
    public ResponseEntity<Employee> getEmployeeById(@PathVariable("id") Long id)
    {
        ...
        try
        {
            emp=this.adminService.getEmployeeByIdService(id);
            if(emp==null)ResponseEntity.status(HttpStatus.NOT_FOUND).build();
            return new ResponseEntity<>(this.adminService.getEmployeeByIdService(id),HttpStatus.OK);
        }
        catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    ...
    {
        try {
            List<Employee> emps = this.adminService.getEmployeesByDepartmentService(department);
            if (emps.size() == 0) return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
            return new ResponseEntity<>(emps, HttpStatus.OK);
        }
        catch (Exception e)
    ...
    public ResponseEntity<Employee> createEmployee(@RequestBody Employee emp)
    {
        try {
            ...
            Employee employee = this.adminService.createEmployeeService(emp);
    ...
    public ResponseEntity<List<Employee>> createEmployees(@RequestBody List<Employee> emps)
    {
        try {
            ...
            List<Employee> employees = this.adminService.createEmployeesService(emps);
    ...
    public ResponseEntity<Employee> updateEmployee(@RequestBody Employee emp,@PathVariable("id") Long id)
    {
        try {
            emp.setPassword(passwordEncoder.encode(emp.getPassword()));
            Employee employee = this.adminService.updateEmployeeService(emp, id);
            if (employee == null) ResponseEntity.status(HttpStatus.BAD_REQUEST).build();
            return new ResponseEntity<>(this.adminService.updateEmployeeService(emp, id), HttpStatus.OK);
        }
        catch (Exception e)
        {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
    ...
    {
        try {
            this.adminService.deleteEmployeeByIdService(id);
    ...
    {
        try {
            this.adminService.deleteEmployeesService();
            return new ResponseEntity<>("Deleted Successfully", HttpStatus.OK);
        }
        catch (Exception e)
        {
